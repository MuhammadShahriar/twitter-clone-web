...
function FollowRow({ u, meHandle }: { u: UserListItem; meHandle?: string }) {
  const router = useRouter();
  const bareHandle = bare(u.handle);
  // Don't offer to "follow yourself" — hide the pill on your own row.
  const isMe = meHandle != null && bare(meHandle) === bareHandle;
  const profileHref = `/${bareHandle}`;
  // The whole row navigates to the profile; the inner links + the FollowButton
  // stop propagation so they act on their own target, not the row.
    ...
    <>
      ...
      {status === "ready" && profile && (
        <section aria-label={mode === "followers" ? "Followers" : "Following"}>
          ...
          {!listLoading &&
            items.map((u) => (
              <FollowRow key={u.id} u={u} meHandle={user?.handle} />
            ))}
